      ...
      <motion.div variants={itemVariants}>
        <Card className="p-4 mb-6 bg-white border-orange-200">
          <h2 className="text-lg font-semibold text-orange-800 mb-2">
            {currentMonth} Feed Status
          </h2>

          {!currentData ? (
            <p className="text-orange-600">
              No data for current month. Please add a report.
            </p>
          ) : (
            <>
              ...
              <div className="bg-orange-50 p-3 rounded-md">
                <p className="text-sm text-orange-700">
                  <span className="font-semibold">
                    {currentData.chickens} chickens
                  </span>{" "}
                  require approximately{" "}
                  <span className="font-semibold">
                    {calculateFeedNeeded(currentData.chickens)}kg
                  </span>{" "}
                  of feed per month
                </p>
              </div>
      ...
      <motion.div variants={itemVariants} className="mb-4">
        ...
        <div className="bg-white p-4 rounded-lg border border-orange-200">
          {monthlyData.length === 0 ? (
            <p className="text-orange-600 text-center py-6">
              No data available
            </p>
          ) : (
